...
export async function fetchNumberOfInvoicesAndCustomers() {
  try {
    ...
    return {
      numberOfInvoices: Number(invoiceCountPromise[0].count ?? "0"),
      numberOfCustomers: Number(customerCountPromise[0].count ?? "0"),
    };
  } catch (error) {
    console.error("Database Error:", error);
